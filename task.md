...
> 本文档只登记**未做**的功能，按 design.md 的里程碑组织（框架层面的已知限制与 TODO 小项见 [framework.md](framework.md) §8）。架构已为每项留好接口（格式注册表 / Job 引擎占位 / 持久化层）。每项标注:文件类型说明、接入管道(native 前端库 / decode-rust / ffmpeg-stream / archive 双身份容器)、推荐库。架构不变:加一个格式 = 加一个 handler 文件 + registry.ts 登记一行(design.md §5②)。
...
3D 与图片类别已上线(单任务、按钮级 busy,一律先确认弹窗,无队列/进度);其余类别仍占位(架构已按 design.md §4 预留)。
