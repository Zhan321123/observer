...
interface VideoSeekBarProps {
  ...
  /** true=拖动即 seek(原生,便宜);false=松手才 seek(流式,防逐像素重启 ffmpeg) */
  live?: boolean;
}

/**
 * 视频进度条(带悬停预览,M1"进度条缩略图"+ §新增振幅波形):
 * 拖动=seek、悬停=该时间点海报帧 + 时间、轨道=音轨振幅波形(WaveformSeekBar)。
 * 悬停帧经 `video_thumbnail(path, 秒)` 生成(后端按秒分桶磁盘缓存);取帧按 90ms 防抖,
 * 避免快速划过时每像素都拉起 ffmpeg。无音轨时 WaveformSeekBar 自动回退普通进度条。
 */
export function VideoSeekBar({ path, duration, value, onSeek, live = true }: VideoSeekBarProps) {
  ...
  return (
    <div ref={wrapRef} className="relative min-w-0 flex-1" onMouseMove={onMove} onMouseLeave={onLeave}>
      ...
      {/* 轨道=音轨振幅波形(无音轨/静音/失败自动回退普通条);拖动语义经 live 传入 */}
      <WaveformSeekBar
        path={path}
        duration={duration}
        value={value}
